...
class TestEntityCardContract:
    ...
    def test_entity_card_retrieved_by_name(self, retriever):
        rq = RetrievalQuery(
            original_query="SMF",
...
class TestGeneratedQuestionContract:
    ...
    def test_generated_question_retrieved(self, retriever):
...
class TestEntityRefsJsonContract:
    ...
    def test_entity_refs_parsed(self, retriever):
        rq = RetrievalQuery(original_query="SMF", keywords=["SMF"])
        snapshot_ids = ["aaaa0000-0000-0000-0000-000000000003"]
        results = retriever.retrieve(rq, snapshot_ids)
...
class TestSourceRefsJsonContract:
    ...
    def test_source_refs_in_metadata(self, retriever):
        rq = RetrievalQuery(original_query="SMF", keywords=["SMF"])
        snapshot_ids = ["aaaa0000-0000-0000-0000-000000000003"]
        results = retriever.retrieve(rq, snapshot_ids)
        for r in results:
