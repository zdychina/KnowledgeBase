...
class TestEntityExactRetriever:
    def test_retrieve_by_entity(self, retriever):
        ...
        results = retriever.retrieve(rq, snapshot_ids)
        assert len(results) > 0
        assert any("SMF" in r.metadata.get("text", "") for r in results)

    def test_entity_refs_json_matching(self, retriever):
        rq = RetrievalQuery(original_query="SMF", keywords=["SMF"])
        snapshot_ids = ["aaaa0000-0000-0000-0000-000000000003"]
        results = retriever.retrieve(rq, snapshot_ids)
        assert len(results) > 0

    def test_source_marker(self, retriever):
        rq = RetrievalQuery(original_query="SMF", keywords=["SMF"])
        snapshot_ids = ["aaaa0000-0000-0000-0000-000000000003"]
        results = retriever.retrieve(rq, snapshot_ids)
        for r in results:
    ...
    def test_score_chain(self, retriever):
        rq = RetrievalQuery(
            original_query="SMF",
    ...
    def test_no_results_for_unknown_entity(self, retriever):
    ...
    def test_scope_pushdown_filters(self, retriever):
        ...
        results_all = retriever.retrieve(rq_no_scope, [SNAP_FEATURE, SNAP_UDG])
        results_scoped = retriever.retrieve(rq_scope, [SNAP_FEATURE, SNAP_UDG])
