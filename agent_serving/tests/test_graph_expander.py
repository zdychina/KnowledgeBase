...
class TestGraphExpand:
    def test_expand_from_seed_returns_neighbors(self, graph):
        """Expand from RS_ADD_APN_UDG should find RS_5G_CONCEPT via 'next' relation."""
        results = graph.expand(
    ...
    def test_expand_depth_metadata(self, graph):
        results = graph.expand(
            seed_segment_ids=[RS_ADD_APN_UDG],
            max_depth=2,
            snapshot_ids=[SNAP_UDG],
    ...
    def test_expand_max_results_truncation(self, graph):
        results = graph.expand(
            seed_segment_ids=[RS_ADD_APN_UDG],
            max_depth=2,
            max_results=1,
    ...
    def test_expand_empty_seeds(self, graph):
        results = graph.expand(
            seed_segment_ids=[],
            max_depth=2,
        )
    ...
    def test_expand_relation_types_filter(self, graph):
...
class TestFetchExpandedSegments:
    def test_fetch_expanded_with_metadata(self, graph):
        expansions = [{
        ...
        results = graph.fetch_expanded_segments(
    ...
    def test_fetch_expanded_empty(self, graph):
        results = graph.fetch_expanded_segments([])
        assert results == []
...
class TestGetNeighbors:
    def test_get_neighbors_with_snapshot_filter(self, graph):
        neighbors = graph._get_neighbors(
    ...
    def test_get_neighbors_without_snapshot(self, graph):
    ...
    def test_get_neighbors_with_relation_type_filter(self, graph):
        neighbors = graph._get_neighbors(
            segment_ids=[RS_ADD_APN_UDG],
            relation_types=["next"],
            snapshot_ids=[SNAP_UDG],
    ...
    def test_get_neighbors_empty_segment_ids(self, graph):
        neighbors = graph._get_neighbors(segment_ids=[])
        assert neighbors == []
