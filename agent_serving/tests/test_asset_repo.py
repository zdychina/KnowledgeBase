...
class TestResolveActiveScope:
    def test_active_scope_found(self, repo, seed_ids):
    ...
    def test_no_active_release(self, pg_pool):
    ...
    def test_multiple_active_releases(self, pg_pool):
        with pg_pool.connection() as conn:
...
class TestResolveSourceSegments:
    def test_valid_source_refs(self, repo, seed_ids):
        source_refs = json.dumps({"raw_segment_ids": [seed_ids["rs_add_apn_udg"]]})
        segments = repo.resolve_source_segments(source_refs)
        assert len(segments) == 1
        assert segments[0]["id"] == seed_ids["rs_add_apn_udg"]
        assert segments[0]["raw_text"] is not None

    def test_empty_source_refs(self, repo):
        segments = repo.resolve_source_segments(None)
        assert segments == []

    def test_malformed_json(self, repo):
        segments = repo.resolve_source_segments("not json")
        assert segments == []

    def test_multiple_segments(self, repo, seed_ids):
...
class TestGetRelations:
    def test_relations_for_segments(self, repo, seed_ids):
        relations = repo.get_relations_for_segments(
            [seed_ids["rs_add_apn_udg"]],
        )
        assert len(relations) >= 1
        assert any(r["relation_type"] == "next" for r in relations)

    def test_empty_segment_list(self, repo):
        relations = repo.get_relations_for_segments([])
        assert relations == []

    def test_relation_type_filter(self, repo, seed_ids):
        relations = repo.get_relations_for_segments(
            [seed_ids["rs_add_apn_udg"]],
            relation_types=["next"],
        )
...
class TestGetDocumentSources:
    def test_fetch_documents(self, repo, seed_ids):
        sources = repo.get_document_sources([seed_ids["doc_udg"]])
        assert len(sources) >= 1
        assert any(s["document_key"] == "UDG_OM_REF" for s in sources)

    def test_empty_ids(self, repo):
        sources = repo.get_document_sources([])
        assert sources == []

    def test_documents_filtered_by_snapshot(self, repo, seed_ids):
...
class TestBuildViewConsistency:
    ...
    def test_removed_selection_excluded_from_scope(self, pg_pool):
    ...
    def test_source_segments_filtered_by_snapshot(self, repo, seed_ids):
        """resolve_source_segments should only return segments in active snapshots."""
        source_refs = json.dumps({"raw_segment_ids": [seed_ids["rs_add_apn_udg"]]})
        segments = repo.resolve_source_segments(
            source_refs,
            snapshot_ids=[seed_ids["snap_udg"]],
        )
        assert len(segments) == 1
