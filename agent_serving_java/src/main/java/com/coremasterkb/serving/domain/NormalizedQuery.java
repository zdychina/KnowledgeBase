...
public record NormalizedQuery(
        ...
        @JsonProperty("evidence_need")  EvidenceNeed evidenceNeed
) {
    public NormalizedQuery {
        ...
        if (evidenceNeed  == null) evidenceNeed  = new EvidenceNeed();
    }

    public NormalizedQuery withScope(Map<String, Object> newScope) {
        return new NormalizedQuery(originalQuery, intent, entities, newScope,
                keywords, desiredRoles, evidenceNeed);
    }

    public NormalizedQuery withEntities(List<EntityRef> newEntities) {
        return new NormalizedQuery(originalQuery, intent, newEntities, scope,
                keywords, desiredRoles, evidenceNeed);
    }
}
